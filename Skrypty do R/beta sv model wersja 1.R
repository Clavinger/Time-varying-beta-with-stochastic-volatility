...
L.beta.log<- foreach(i=1:length(xx1) ,.packages='pomp', .export = "betalist",.combine=rbind,
                     .options.multicore=list(set.seed=TRUE)) %dopar% {
                       ...
                       logLik(pfilter(beta.filt,params=c(mu_h=xx1[i], 
                                                         ...
                                                         sigma_gamma =as.numeric(params_nowe2['sigma_gamma']),
                                                         Beta_0=as.numeric(params_nowe2['Beta_0']),
                                                         Alpha_0=as.numeric(params_nowe2['Alpha_0']),
...
#parametr sigma_gamma
xx5<-seq(from=.0001,to=.01,length.out = 100)
detectCores()
cl <- makeCluster(3, type = "SOCK")
registerDoSNOW(cl)
...
plot(xx5, L.beta.log5, type='l',xlab=expression(sigma[gamma]),ylab="logLik")
points(xx5, L.beta.log5)
points(r.box[,'sigma_gamma'], r.box[,'logLik'] ,col='red')
p5=loess(L.beta.log5~xx5,span=0.5)
lines(xx5,p5$fitted,col='blue',lwd=2)
abline(v=params_nowe2['sigma_gamma'],lty=2)
beep(2)
...
#parametr Alpha_0
xx8<-seq(from=-.1,to=.5,length.out = 100)
detectCores()
cl <- makeCluster(3, type = "SOCK")
registerDoSNOW(cl)
L.beta.log8<- foreach(i=1:length(xx8) ,.packages='pomp', .export = "betalist",.combine=rbind,
                      .options.multicore=list(set.seed=TRUE)) %dopar% {
                        set.seed(87932)
                        logLik(pfilter(beta.filt,params=c(mu_h=as.numeric(params_nowe2['mu_h']), 
                                                          phi=as.numeric(params_nowe2['phi']),
                                                          ...
                                                          Beta_0=as.numeric(params_nowe2['Beta_0']),
                                                          Alpha_0=xx7[i],
                                                          H_0=as.numeric(params_nowe2['Alpha_0'])),
                                       Np=betalist[[1]][run_level] ))
                      }
...
plot(xx8, L.beta.log8, type='l',xlab=expression(alpha[0]),ylab="logLik")
points(xx8, L.beta.log8)
points(r.box[,'H_0'], r.box[,'logLik'] ,col='red')
p8=loess(L.beta.log8~xx8,span=0.5)
lines(xx8,p8$fitted,col='blue',lwd=2)
abline(v=params_nowe2['Alpha_0'],lty=2)
